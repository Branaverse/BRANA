...
/*************************
BRANAVERSE Private-Sale WALLET
*************************/
...
contract BranaPrivateSaleWallet {
    ...
    uint256 public privateSaleVault;
    ...
    mapping(address => PrivateSaleWallet) public Receiver;
    ...
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        ...
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(privateSaleVault);
    ...
    function claimMonthlyAmount(address _receiver) external isAdminHolder(msg.sender) nonReentrant{
        ...
        privateSaleVault -= amountAllowed;
        if(Receiver[_receiver].amount == 0){
    ...
    function withdrawalBRANA(uint256 _amount, uint256 decimal, address to) external onlyOwner() {
        ...
        require(amount > 0 && _amount*dcml <= amount, "No BRANA!");// can only withdraw what is not locked for Private Sale Wallet.
