...
contract BranaCompanyWallet {
    ...
    event CompanyWalletAdded(address Wallet, uint256 Amount);
    ...
    mapping(address => CompanyWallet) public Receiver;
    ...
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        ...
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(companyVault);
        ...
        emit CompanyWalletAdded(_receiver, _amount);
    ...
    function withdrawalBRANA(uint256 _amount, uint256 decimal, address to) external onlyOwner() {
        uint256 amount = BRANA.balanceOf(address(this)).sub(companyVault);
        require(amount > 0 && _amount <= amount, "No BRANA!");// can only withdraw what is not locked for Company Wallet.
