...
contract BranaLiquidityWallet {
    ...
    event LiquidityWalletAdded(address Wallet, uint256 Amount);
    ...
    mapping(address => LiquidityWallet) public Receiver;
    ...
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        ...
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(liquidityVault);
        ...
        if(Receiver[_receiver].amount > 0){
            ...
            liquidityVault += _amount;
            return;
        }
        emit LiquidityWalletAdded(_receiver, _amount);
