...
/*****************************
BRANAVERSE Secured Funds WALLET
*****************************/
...
contract BranaSecuredFundsWallet {
    ...
    uint256 public securedFundsVault;
    ...
    event SecuredFundsWalletAdded(address Wallet, uint256 Amount);
    ...
    mapping(address => SecuredFundsWallet) public Receiver;
    ...
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        ...
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(securedFundsVault);
        ...
        if(Receiver[_receiver].amount > 0){
            ...
            securedFundsVault += _amount;
            return;
        }
        emit SecuredFundsWalletAdded(_receiver, _amount);
    ...
    function claimMonthlyAmount(address _receiver) external isAdminHolder(msg.sender) nonReentrant{
        ...
        securedFundsVault -= amountAllowed;
        if(Receiver[_receiver].amount == 0){
    ...
    function claimRemainings(address _receiver) external isAdminHolder(msg.sender) nonReentrant{
        ...
        securedFundsVault -= remainAmount;
        delete Receiver[_receiver];
    ...
    function withdrawalBRANA(uint256 _amount, uint256 decimal, address to) external onlyOwner() {
        uint256 amount = BRANA.balanceOf(address(this)).sub(securedFundsVault);
        uint256 dcml = 10 ** decimal;
        require(amount > 0 && _amount*dcml <= amount, "No BRANA!");// can only withdraw what is not locked for Secured Funds
