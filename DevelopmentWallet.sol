...
contract BranaDevelopmentWallet {
    ...
    uint256 public developmentVault;
    ...
    event DevelopmentWalletAdded(address Wallet, uint256 Amount);
    ...
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        ...
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(developmentVault);
        ...
        if(Receiver[_receiver].amount > 0){
            ...
            developmentVault += _amount;
            return;
        }
        emit DevelopmentWalletAdded(_receiver, _amount);
    ...
    function claimMonthlyAmount(address _receiver) external isAdminHolder(msg.sender) nonReentrant{
        ...
        developmentVault -= amountAllowed;
        if(Receiver[_receiver].amount == 0){
    ...
    function withdrawalBRANA(uint256 _amount, uint256 decimal, address to) external onlyOwner() {
        uint256 amount = BRANA.balanceOf(address(this)).sub(developmentVault);
        uint256 dcml = 10 ** decimal;
        require(amount > 0 && _amount*dcml <= amount, "No BRANA!");// can only withdraw what is not locked for Development.
