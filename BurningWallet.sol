...
contract BranaBurnWallet {
    ...
    event BurnWalletAdded(address Wallet, uint256 Amount);
    event BRANABurned(address Wallet, address Operator, uint256 Amount);
    ...
    mapping(address => BurnWallet) public Receiver;
    ...
    @dev once this function is executed tokens are considered burned
    nobody can retrieve tokens from burnVault, tokens can only go
    to a dead address.
    */
    function lockWallet(address _receiver, uint256 _amount, uint256 _lockTime) external onlyOwner{
        require(_receiver == deadAddress,"This is not a Burn Wallet!");
        uint256 availableAmount = BRANA.balanceOf(address(this)).sub(burnVault);
        ...
        emit BurnWalletAdded(_receiver, _amount);
    ...
    function claimMonthlyAmount() external isAdminHolder(msg.sender) nonReentrant{
        uint256 totalTimeLock = Receiver[deadAddress].monthLock;
        uint256 mainAmount = Receiver[deadAddress].falseAmount;
        uint256 remainAmount = Receiver[deadAddress].amount;
        ...
        Receiver[deadAddress].amount = remainAmount.sub(amountAllowed);
        Receiver[deadAddress].monthLock += monthly;
        burnVault -= amountAllowed;
        if(Receiver[deadAddress].amount == 0){
            delete Receiver[deadAddress]; 
            walletCount--;
        }
        emit BRANABurned(deadAddress, msg.sender, amountAllowed);
        BRANA.transfer(deadAddress, amountAllowed);
    }
    function claimRemainings() external isAdminHolder(msg.sender) nonReentrant{
        uint256 fullTime = hPercent.div(mP).mul(monthly);
        uint256 totalTimeLock = Receiver[deadAddress].lockTime.add(fullTime);
        require(totalTimeLock <= block.timestamp, "Not yet");
        uint256 remainAmount = Receiver[deadAddress].amount;
        Receiver[deadAddress].amount = 0;
        burnVault -= remainAmount;
        delete Receiver[deadAddress];
        emit BRANABurned(deadAddress, msg.sender, remainAmount);
        BRANA.transfer(deadAddress, remainAmount);
        walletCount--;
    }
    ...
    function withdrawalBRANA(uint256 _amount, uint256 decimal, address to) external onlyOwner() {
        uint256 amount = BRANA.balanceOf(address(this)).sub(burnVault);
        uint256 dcml = 10 ** decimal;
        require(amount > 0 && _amount*dcml <= amount, "No BRANA!");// can only withdraw what is not locked for Burning.
